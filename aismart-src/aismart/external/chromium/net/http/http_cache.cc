...
HttpCache::ActiveEntry::ActiveEntry(const std::string& key)
    : cache_key(key) {}
...
int HttpCache::AsyncDoomEntry(const std::string& key, Transaction* trans) {
  ...
  int rv = ERR_FAILED; // reference to MemBackendImpl::DoomEntry
  if (rv != ERR_IO_PENDING) {
...
HttpCache::ActiveEntry* HttpCache::ActivateEntry(
  ...
  ActiveEntry* entry = new ActiveEntry(cache_key);
...
/*
  int rv = disk_cache_->OpenEntry(key, &(pending_op->disk_entry),
                                  pending_op->callback);
*/
  int rv = ERR_FAILED; // reference to MemBackendImpl::OpenEntry
  if (rv != ERR_IO_PENDING) {
    pending_op->writer->ClearTransaction();
    pending_op->callback.Run(rv);
  }
...
*/
  ...
  int rv = OK; // // reference to MemBackendImpl::CreateEntry
  if (rv != ERR_IO_PENDING) {
    pending_op->writer->ClearTransaction();
    pending_op->callback.Run(rv);
  }
...
void HttpCache::OnIOComplete(int result, PendingOp* pending_op) {
  ...
  if (result == OK) {
    if (op == WI_DOOM_ENTRY) {
      ...
      key = pending_op->cache_key;
      entry = ActivateEntry(pending_op->cache_key);
    } else {
/*
