...
// this file "copy" from url_request_test_util.cc
...
void RoseURLRequestContext::Init() 
{
	...
	if (!host_resolver()) {
		...
		context_storage_.set_host_resolver(
			std::unique_ptr<HostResolver>(new HostResolverImpl(options, &net_log)));
	...
	if (!transport_security_state()) {
		context_storage_.set_transport_security_state(std::make_unique<TransportSecurityState>());
	}
	if (!cert_transparency_verifier()) {
		context_storage_.set_cert_transparency_verifier(std::make_unique<DoNothingCTVerifier>());
	}
	if (!ct_policy_enforcer()) {
		context_storage_.set_ct_policy_enforcer(std::make_unique<CTPolicyEnforcer>());
	}
	if (!ssl_config_service()) {
		SSLConfigService* ssl_config = nullptr;
		if (cert_.empty()) {
			ssl_config = new SSLConfigServiceDefaults();
		} else {
			ssl_config = new SSLConfigServiceBindCert(cert_);
		}
		context_storage_.set_ssl_config_service(ssl_config);
	}
	if (!http_auth_handler_factory()) {
		context_storage_.set_http_auth_handler_factory(HttpAuthHandlerFactory::CreateDefault(host_resolver()));
	}
	if (!http_server_properties()) {
		context_storage_.set_http_server_properties(std::unique_ptr<HttpServerProperties>(new HttpServerPropertiesImpl()));
	}
	/*
	...
	*/
	...
	if (!channel_id_service()) {
		context_storage_.set_channel_id_service(std::make_unique<ChannelIDService>(new DefaultChannelIDStore(nullptr)));
	}
	...
	// EXPECT_FALSE(client_socket_factory_);
		...
		context_storage_.set_http_network_session(std::make_unique<HttpNetworkSession>(session_params, session_context));
		context_storage_.set_http_transaction_factory(std::make_unique<HttpCache>(context_storage_.http_network_session(),
...
/*
	...
	*/
	...
	if (!job_factory()) {
		context_storage_.set_job_factory(std::make_unique<URLRequestJobFactoryImpl>());
	}
}
...
void RoseDelegate::ClearFullRequestHeaders() {
...
void RoseDelegate::OnReadCompleted(URLRequest* request, int bytes_read) 
{
	...
	if (bytes_read >= 0) {
		...
		if (executor_.progress && expected_size > 0) {
			executor_.progress->set_percentage(100 * received_bytes_count_ / expected_size);
...
void RoseDelegate::OnSendBodyCompleted(URLRequest* request, int sofar_bytes, int expected_bytes)
{
	...
	if (executor_.progress && expected_bytes > 0) {
		executor_.progress->set_percentage(100 * sofar_bytes / expected_bytes);
...
void TestNetworkDelegate::OnStartTransaction(
  ...
  // EXPECT_TRUE(next_states_[req_id] & kStageStartTransaction) << event_order_[req_id];
